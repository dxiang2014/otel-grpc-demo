...
logging.debug(f"Converted timestamp: {datetime.utcfromtimestamp(custom_timestamp_ns / 1_000_000_000)} UTC")

# Send to otel-collector
response = requests.post("http://otel-collector:4318/v1/metrics", data=payload, headers=headers)

logging.debug(f"Sent metric with timestamp: {custom_timestamp_ns} ({datetime.utcfromtimestamp(custom_timestamp_ns / 1_000_000_000)} UTC)")
